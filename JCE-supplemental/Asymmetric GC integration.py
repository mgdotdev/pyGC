...
in order to find the set of gaussians which best fit the data (there are a few caveats 
...
# It sould be noted that the parameters don't fit the strict definitions as defined
...
initials = [6.5, 13, 1, 0], [4.5, 19, 1, 0]
...
def gaussian(x,a,b,c,d):
    return (a / (c * np.sqrt(2 * np.pi))) * np.exp((-(x - b) ** 2.0) / 2 * c ** 2.0) * (1 + erf((d * (x - b)) / (c * np.sqrt(2))))

# defines the expected resultant as a sum of intrinsic gaussian functions
def GaussSum(x, p):
    return sum(gaussian(x, p[4*k], p[4*k+1], p[4*k+2], p[4*k+3]) for k in range(n))

# defines a residual, which is the  reducing the square of the difference between the data (y) and the function 'func(x,p)'
def residuals(p, y, x):
    return np.abs((y - GaussSum(x,p)))
...
areas = [quad(gaussian, M[0,0], M[M.shape[0]-1,0],
        args=(cnst[4*i], cnst[4*i+1], cnst[4*i+2], cnst[4*i+3]))[0] 
        for i in range(n)]
    
# defines the independent variable. 
x = np.linspace(0,40,200)

# visualization block; creates and formats a figure variable to place the data in.
